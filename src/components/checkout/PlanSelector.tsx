      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 bg-gray-800">
        {plans.map(plan => <Card key={plan.id} className={`relative cursor-pointer transition-all duration-200 ${selectedPlan?.id === plan.id ? 'ring-2 ring-primary bg-primary/5' : 'hover:ring-1 hover:ring-primary/50'} ${plan.best_seller ? 'border-primary' : 'border-gray-700'}`} onClick={() => onSelectPlan(plan)}>
            ...
            <CardHeader className="pb-4 bg-gray-800">
              ...
              {plan.description && <CardDescription className="text-gray-400">
                  {plan.description}
                </CardDescription>}
            </CardHeader>
            ...
            <CardContent className="space-y-4 bg-gray-800">
              <div className="text-center">
                ...
                {plan.billing_cycle === 'annually' && <p className="text-gray-400 text-sm">
                    Total de {formatCurrency(plan.price * 12)}
                  </p>}
              </div>

              {plan.benefits && plan.benefits.length > 0 && <div className="space-y-2">
                  <h4 className="text-white font-medium text-sm">Benefícios inclusos:</h4>
                  <ul className="text-gray-300 text-sm space-y-1">
                    {plan.benefits.map((benefit, index) => <li key={index} className="flex items-start gap-2">
              ...
              <Button className={`w-full ${selectedPlan?.id === plan.id ? 'bg-primary text-primary-foreground' : 'bg-gray-700 text-white hover:bg-gray-600'}`} onClick={e => {
            e.stopPropagation();
            onSelectPlan(plan);
          }}>
