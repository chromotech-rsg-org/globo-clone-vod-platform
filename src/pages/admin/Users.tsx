...
import { Search, Edit, Eye, Save, X, Plus, Trash2 } from 'lucide-react';
...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const filteredData = isDeveloper 
        ? data || []
        : (data || []).filter(user => user.role !== 'desenvolvedor');
  ...
  const handleEdit = (user: UserProfile) => {
    // Only developers can edit developers
    if (user.role === 'desenvolvedor' && !isDeveloper) {
      toast({
        title: "Acesso Negado",
        description: "Apenas desenvolvedores podem editar usuários desenvolvedores",
        variant: "destructive"
      });
      return;
    }
  ...
  const handleView = (user: UserProfile) => {
    // Only developers can view developers
    if (user.role === 'desenvolvedor' && !isDeveloper) {
      toast({
        title: "Acesso Negado",
        description: "Apenas desenvolvedores podem visualizar usuários desenvolvedores",
        variant: "destructive"
      });
      return;
    }
  ...
  const handleCreateSave = async () => {
    ...
    if (formData.role === 'desenvolvedor' && !isDeveloper) {
      toast({
        title: "Acesso Negado",
        description: "Apenas desenvolvedores podem criar usuários desenvolvedores",
        variant: "destructive"
      });
      return;
    }
  ...
  const handleSave = async () => {
    ...
    // Only developers can change role to/from developer
    if ((formData.role === 'desenvolvedor' || editingUser.role === 'desenvolvedor') && !isDeveloper) {
      toast({
        title: "Acesso Negado",
        description: "Apenas desenvolvedores podem gerenciar usuários desenvolvedores",
        variant: "destructive"
      });
      return;
    }
  ...
  const handleDelete = async (userId: string, userRole: string) => {
    ...
    if (userRole === 'desenvolvedor' && !isDeveloper) {
      toast({
        title: "Acesso Negado",
        description: "Apenas desenvolvedores podem excluir usuários desenvolvedores",
        variant: "destructive"
      });
      return;
    }
