...
const Subscription = () => {
  ...
  return <div className="p-6 space-y-6">
      ...
      {subscription ? <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {subscription.plan.benefits && subscription.plan.benefits.length > 0 && <div>
                  <h4 className="font-medium mb-2">Benefícios:</h4>
                  <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                    {subscription.plan.benefits.map((benefit, index) => <li key={index}>• {benefit}</li>)}
                  </ul>
                </div>}

              <div className="flex space-x-3">
                {subscription.status === 'active' ? <Button variant="destructive" onClick={handleCancelSubscription}>
                    Cancelar Assinatura
                  </Button> : <Button onClick={handleReactivateSubscription}>
                    Reativar Assinatura
                  </Button>}
              </div>
            </CardContent>
          ...
          {subscription.status === 'active' && <Card>
              <CardHeader>
                <CardTitle>Mudar de Plano</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {availablePlans.filter(plan => plan.id !== subscription.plan_id).map(plan => <div key={plan.id} className="border rounded-lg p-4 space-y-3">
                      <div>
                      ...
                      <Button variant="outline" size="sm" onClick={() => handleUpgradePlan(plan.id)} className="w-full">
