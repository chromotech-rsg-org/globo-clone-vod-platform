-- Modificar o trigger validate_bid_value para aprovar automaticamente os lances
...
-- Drop the old validate_bid_value trigger
DROP TRIGGER IF EXISTS validate_bid_value_trigger ON bids;

-- Create new trigger for validation and auto-approval
CREATE TRIGGER validate_and_approve_bid_trigger
  BEFORE INSERT ON bids
  FOR EACH ROW
  EXECUTE FUNCTION validate_and_approve_bid();

-- Update default bid status to 'approved' (though the trigger will set it anyway)
ALTER TABLE bids ALTER COLUMN status SET DEFAULT 'approved';